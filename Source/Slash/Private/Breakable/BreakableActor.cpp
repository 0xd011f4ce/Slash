...
ABreakableActor::ABreakableActor ()
{
  ...
  CapsuleComponent->SetCollisionResponseToAllChannels (
...
void
ABreakableActor::BeginPlay ()
{
  Super::BeginPlay ();
}

void
ABreakableActor::Tick (float DeltaTime)
{
  Super::Tick (DeltaTime);
}

void
ABreakableActor::GetHit_Implementation (const FVector &ImpactPoint, AActor *Hitter)
{
  if (bBroken)
    return;
  ...
  if (World && TreasureClasses.Num () > 0)
    {
      ...
      World->SpawnActor<ATreasure> (TreasureClasses[Selection], Location,
                                    GetActorRotation ());
    }
}
